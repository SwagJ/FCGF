...
class ContextNet(ME.MinkowskiNetwork):
  NORM_TYPE = 'BN'
  BLOCK_NORM_TYPE = 'BN'
  ...
  def __init__(self,
    ...
    self.context1 = Context(CHANNELS[1],bn_momentum)
    ...
    self.context2 = Context(CHANNELS[2],bn_momentum)
    ...
    self.context3 = Context(CHANNELS[3],bn_momentum)
    ...
    self.context4 = Context(CHANNELS[4],bn_momentum)
...
class Context(ME.MinkowskiNetwork):
  def __init__(self,
               in_channels=3,
               bn_momentum=0.1,
               D=3):
    ...
    self.conv = ME.MinkowskiConvolution(
        ...
        has_bias=False,
        dimension=D)
    self.norm = get_norm('BN', in_channels, bn_momentum=bn_momentum, D=D)
    self.pool = ME.MinkowskiGlobalPooling()

  def forward(self, x):
    out = self.pool(x).F.mean(0).unsqueeze(0)
    print("out shape:", out.shape)
    out = torch.cat([out]*x.C.shape[0],dim=0)

    out = ME.SparseTensor(out,coords_key=x.coords_key,coords_manager=x.coords_man)
    out = self.conv(out)
    out = self.norm(out)
    out = MEF.sigmoid(out)
